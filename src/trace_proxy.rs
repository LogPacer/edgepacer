...
impl TraceService for TraceGrpcService {
    async fn export(
        ...
        match process_trace_request(self.state.as_ref(), request.into_inner()).await {
            ...
            Err(TraceRequestRejection::ServiceNamePolicy(_)) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn start_fails_loudly_when_grpc_address_is_unavailable() {
        // Hold the gRPC port so the bind cannot succeed — a busy port must fail
        // start() instead of leaving a silent dead listener inside the task.
        let occupied = std::net::TcpListener::bind("127.0.0.1:0").unwrap();
        let grpc_address = occupied.local_addr().unwrap();

        let dir = tempfile::tempdir().unwrap();
        let mut config = test_config(
            "http://localhost:9999/v1/logpacer-wire".into(),
            &dir.path().join("trace-buffer.sqlite"),
        );
        config.grpc_listen_address = Some(grpc_address);
        ...
        assert!(
            result.is_err(),
            "an unavailable gRPC port must fail start, not start silently"
        );
        // Tear down the HTTP server the failed start already spawned.
        proxy.stop().await;
    }

    #[tokio::test]
    async fn grpc_export_buffers_on_retryable_upstream_failure() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path(WIRE_PATH))
            .respond_with(ResponseTemplate::new(503))
        ...
        // A retryable upstream failure still reports success to the exporter —
        // the span is buffered for replay, mirroring the HTTP path.
        let response = service
            .export(tonic::Request::new(request))
            .await
            .expect("gRPC export should accept the request despite an upstream failure");
        assert_eq!(response.into_inner(), ExportTraceServiceResponse::default());
